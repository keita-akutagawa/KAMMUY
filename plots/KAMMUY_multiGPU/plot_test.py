...
procs = 1
buffer = 3
dirname = "/cfca-work/akutagawakt/KAMMUY_multiGPU/results_test"
step = 10
savename = f"test.png"

total_field_pic = np.zeros([nx_pic + 6, ny_pic + 6])
total_field_mhd = np.zeros([nx_mhd + 6, ny_mhd + 6])
for rank in range(procs):
    ...
    filename = f"{dirname}/test_first_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
        first_moment_ion = np.fromfile(f, dtype=np.float32)
    first_moment_ion = first_moment_ion.reshape(local_nx_pic, local_ny_pic, 3).T
    filename = f"{dirname}/test_first_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/test_second_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/test_second_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/test_U_lower_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/test_U_upper_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
...
print(np.isnan(total_field_pic).sum())
#ax.plot(total_field_pic[10, :])
mappable = ax.pcolormesh(total_field_pic.T, cmap='jet')
#ax.set_ylim(1, ny_pic + 6)
#mappable = ax.pcolormesh(total_field_mhd.T, cmap='jet')
#ax.set_ylim(1000, 1200)
#ax.scatter(x_electron[0], x_electron[1], s=1)
#ax.set_ylim(190, 210)

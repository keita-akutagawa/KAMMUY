...
buffer = 3
procs = 1
dirname = "/cfca-work/akutagawakt/KAMMUY_multiGPU/results_alfven"
step = 420
savename = f"{step}.png"
...
for rank in range(procs):
    ...
    filename = f"{dirname}/alfven_first_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
        first_moment_ion = np.fromfile(f, dtype=np.float32)
    first_moment_ion = first_moment_ion.reshape(local_nx_pic, local_ny_pic, 3).T
    filename = f"{dirname}/alfven_first_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/alfven_second_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/alfven_second_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/alfven_U_lower_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/alfven_U_upper_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
...
axs[0].plot(y_coordinate_mhd, u_lower_mhd[:, index] / VA_mhd, color='black', lw=3)
axs[1].plot(y_coordinate_mhd, w_lower_mhd[:, index] / VA_mhd, color='black', lw=3)
axs[2].plot(y_coordinate_mhd, Bx_lower_mhd[:, index] / B0_mhd, color='black', lw=3)
axs[3].plot(y_coordinate_mhd, Bz_lower_mhd[:, index] / B0_mhd, color='black', lw=3)
axs[0].plot(y_coordinate_mhd + 10450, u_upper_mhd[:, index] / VA_mhd, color='black', lw=3)
axs[1].plot(y_coordinate_mhd + 10450, w_upper_mhd[:, index] / VA_mhd, color='black', lw=3)
axs[2].plot(y_coordinate_mhd + 10450, Bx_upper_mhd[:, index] / B0_mhd, color='black', lw=3)
axs[3].plot(y_coordinate_mhd + 10450, Bz_upper_mhd[:, index] / B0_mhd, color='black', lw=3)
axs[0].plot(y_coordinate_pic + 9500, bulk_vx_pic[:, index] / VA_pic, color='green', lw=3)
axs[1].plot(y_coordinate_pic + 9500, bulk_vz_pic[:, index] / VA_pic, color='green', lw=3)
axs[2].plot(y_coordinate_pic + 9500, B[0, :, index] / B0_pic, color='green', lw=3)
axs[3].plot(y_coordinate_pic + 9500, B[2, :, index] / B0_pic, color='green', lw=3)

axs[0].axvspan(0, 9550, color='blue', alpha=0.6)
axs[1].axvspan(0, 9550, color='blue', alpha=0.6)
axs[2].axvspan(0, 9550, color='blue', alpha=0.6)
axs[3].axvspan(0, 9550, color='blue', alpha=0.6)
axs[0].axvspan(10450, 20000, color='blue', alpha=0.6)
axs[1].axvspan(10450, 20000, color='blue', alpha=0.6)
axs[2].axvspan(10450, 20000, color='blue', alpha=0.6)
axs[3].axvspan(10450, 20000, color='blue', alpha=0.6)
axs[0].axvspan(9500, 10500, color='orange', alpha=0.6)
axs[1].axvspan(9500, 10500, color='orange', alpha=0.6)
axs[2].axvspan(9500, 10500, color='orange', alpha=0.6)
axs[3].axvspan(9500, 10500, color='orange', alpha=0.6)

axs[3].set_xlabel(r"$x / \lambda_{De}$", fontsize=28)
axs[0].set_ylabel(r"$u / V_A$", fontsize=28)
axs[1].set_ylabel(r"$w / V_A$", fontsize=28)
axs[2].set_ylabel(r"$B_y / B_0$", fontsize=28)

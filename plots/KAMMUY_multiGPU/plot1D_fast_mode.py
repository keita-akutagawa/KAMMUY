...
buffer = 3
procs = 1
dirname = "/cfca-work/akutagawakt/KAMMUY_multiGPU/results_fast_mode"
step = 500
savename = f"{step}.png"
...
for rank in range(procs):
    ...
    filename = f"{dirname}/fast_mode_first_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
        first_moment_ion = np.fromfile(f, dtype=np.float32)
    first_moment_ion = first_moment_ion.reshape(local_nx_pic, local_ny_pic, 3).T
    filename = f"{dirname}/fast_mode_first_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/fast_mode_second_moment_ion_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/fast_mode_second_moment_electron_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/fast_mode_U_lower_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
    ...
    filename = f"{dirname}/fast_mode_U_upper_{step}_{rank}.bin"
    with open(filename, 'rb') as f:
...
axs[1].plot(y_coordinate_mhd, v_lower_mhd[:, index] / VA_mhd, color='black', lw=3)
...
axs[0].plot(y_coordinate_mhd + 10450, rho_upper_mhd[:, index] / rho0_mhd, color='black', lw=3)
axs[1].plot(y_coordinate_mhd + 10450, v_upper_mhd[:, index] / VA_mhd, color='black', lw=3)
...
axs[0].plot(y_coordinate_pic + 9500, rho_pic[:, index] / rho0_mhd, color='green', lw=3)
axs[1].plot(y_coordinate_pic + 9500, bulk_vy_pic[:, index] / VA_pic, color='green', lw=3)
...
axs[2].grid()
axs[3].grid()
axs[0].set_yticks([0.9, 1.0, 1.1])
axs[1].set_yticks([-0.1, 0.0, 0.1])
axs[2].set_yticks([-0.1, 0.0, 0.1])
axs[3].set_yticks([0.9, 1.0, 1.1])

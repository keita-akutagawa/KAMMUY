...
)
{
    ...
    if (i < IdealMHD2DConst::device_nx && j < IdealMHD2DConst::device_ny) {
        ...
        if (mPIInfo.isInside(i, j)) {
            ...
            double y = (j + mPIInfo.buffer) * IdealMHD2DConst::device_dy; 
            double yCenter = (mPIInfo.localSizeY - Interface2DConst::device_interfaceLength) * IdealMHD2DConst::device_dy
                           + 0.5 * PIC2DConst::device_ny * PIC2DConst::device_dy;
            
            rho = IdealMHD2DConst::device_rho0 * (sqrt(device_betaUpstream) + pow(cosh((y - yCenter) / device_sheatThickness), -2));
            u   = 0.0;
            v   = 0.0;
            w   = 0.0;
            bX  = IdealMHD2DConst::device_B0 * tanh((y - yCenter) / device_sheatThickness); 
            bY  = 0.0;
            bZ  = 0.0;
            p   = IdealMHD2DConst::device_p0 * (device_betaUpstream + pow(cosh((y - yCenter) / device_sheatThickness), -2));
...
)
{
    ...
    if (i < IdealMHD2DConst::device_nx && j < IdealMHD2DConst::device_ny) {
        ...
        if (mPIInfo.isInside(i, j)) {
            ...
            double rho, u, v, w, bX, bY, bZ, e, p;
            double y = (j + mPIInfo.buffer) * IdealMHD2DConst::device_dy
                     + (mPIInfo.localSizeY - Interface2DConst::device_interfaceLength) * IdealMHD2DConst::device_dy
                     + (PIC2DConst::device_ny - Interface2DConst::device_interfaceLength) * PIC2DConst::device_dy; 
            double yCenter = (mPIInfo.localSizeY - Interface2DConst::device_interfaceLength) * IdealMHD2DConst::device_dy
                           + 0.5 * PIC2DConst::device_ny * PIC2DConst::device_dy;
...
int main(int argc, char** argv)
{
    ...
    PIC2DMPI::setupInfo(mPIInfoPIC, buffer, gridX, gridY, mpiBufNumParticles);
    IdealMHD2DMPI::MPIInfo mPIInfoMHD;
    ...
    mPIInfoPIC.yminForProcs = PIC2DConst::ymin + (PIC2DConst::ymax - PIC2DConst::ymin) / mPIInfoPIC.gridX * mPIInfoPIC.localGridX;
    mPIInfoPIC.ymaxForProcs = PIC2DConst::ymin + (PIC2DConst::ymax - PIC2DConst::ymin) / mPIInfoPIC.gridX * (mPIInfoPIC.localGridX + 1);
    ...
    const int totalSubstep = int(round(sqrt(PIC2DConst::mRatio)));// * (PIC2DConst::omegaPe / PIC2DConst::omegaCe)));

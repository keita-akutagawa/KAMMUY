...
namespace PIC2DMPI
{
    ...
    void setupInfo(MPIInfo& mPIInfo, int buffer, int gridX, int gridY, int mpiBufNumParticles);
        ...
        MPI_Datatype mpi_dataType)
    {
        ...
        MPI_Sendrecv(thrust::raw_pointer_cast(sendFieldDown.data()), sendFieldDown.size(), mpi_dataType, down, 0, 
                    thrust::raw_pointer_cast(recvFieldUp.data()),   recvFieldUp.size(),   mpi_dataType, up,   0, 
                    MPI_COMM_WORLD, &st);
        MPI_Sendrecv(thrust::raw_pointer_cast(sendFieldUp.data()),   sendFieldUp.size(),   mpi_dataType, up,   0, 
                    thrust::raw_pointer_cast(recvFieldDown.data()), recvFieldDown.size(), mpi_dataType, down, 0, 
                    MPI_COMM_WORLD, &st);

...
PIC2D::PIC2D()
    : particlesIon     (PIC2DConst::totalNumIon), 
      particlesElectron(PIC2DConst::totalNumElectron),
      E                   (PIC2DConst::nx * PIC2DConst::ny), 
      tmpE                (PIC2DConst::nx * PIC2DConst::ny), 
      B                   (PIC2DConst::nx * PIC2DConst::ny), 
      tmpB                (PIC2DConst::nx * PIC2DConst::ny), 
      current             (PIC2DConst::nx * PIC2DConst::ny), 
      tmpCurrent          (PIC2DConst::nx * PIC2DConst::ny), 
      ...
      secondMomentIon     (PIC2DConst::nx * PIC2DConst::ny), 
      secondMomentElectron(PIC2DConst::nx * PIC2DConst::ny), 

      host_particlesIon     (PIC2DConst::totalNumIon), 
      host_particlesElectron(PIC2DConst::totalNumElectron), 
      host_E                   (PIC2DConst::nx * PIC2DConst::ny),  
      host_B                   (PIC2DConst::nx * PIC2DConst::ny),  
      host_current             (PIC2DConst::nx * PIC2DConst::ny),  
      host_zerothMomentIon     (PIC2DConst::nx * PIC2DConst::ny),  
      host_zerothMomentElectron(PIC2DConst::nx * PIC2DConst::ny),  
      host_firstMomentIon      (PIC2DConst::nx * PIC2DConst::ny),  
      host_firstMomentElectron (PIC2DConst::nx * PIC2DConst::ny),  
      host_secondMomentIon     (PIC2DConst::nx * PIC2DConst::ny),  
      host_secondMomentElectron(PIC2DConst::nx * PIC2DConst::ny)
{
}
...
)
{
    ...
    if ((0 < i) && (i < PIC2DConst::device_nx) && (0 < j) && (j < PIC2DConst::device_ny)) {
...
)
{
    ...
    if (i < PIC2DConst::device_nx - 1 && j < PIC2DConst::device_ny - 1) {
        unsigned long long index = j + PIC2DConst::device_ny * i; 
...
)
{
    ...
    boundaryPIC.freeBoundaryB_x(tmpB);
    boundaryPIC.freeBoundaryB_y(tmpB);
    boundaryPIC.freeBoundaryE_x(tmpE);
    ...
    boundaryPIC.freeBoundaryParticle_x(
    ...
    boundaryPIC.freeBoundaryCurrent_x(tmpCurrent);
    ...
    boundaryPIC.freeBoundaryCurrent_x(current);
    ...
    boundaryPIC.freeBoundaryZerothMoment_x(zerothMomentIon);
    boundaryPIC.freeBoundaryZerothMoment_y(zerothMomentIon);
    boundaryPIC.freeBoundaryZerothMoment_x(zerothMomentElectron);
    boundaryPIC.freeBoundaryZerothMoment_y(zerothMomentElectron);
    boundaryPIC.freeBoundaryFirstMoment_x(firstMomentIon);
    boundaryPIC.freeBoundaryFirstMoment_y(firstMomentIon);
    boundaryPIC.freeBoundaryFirstMoment_x(firstMomentElectron);
    boundaryPIC.freeBoundaryFirstMoment_y(firstMomentElectron);
    boundaryPIC.freeBoundarySecondMoment_x(secondMomentIon);
    boundaryPIC.freeBoundarySecondMoment_y(secondMomentIon);
    boundaryPIC.freeBoundarySecondMoment_x(secondMomentElectron);
...
)
{
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsCurrent.write(reinterpret_cast<const char*>(&host_current[index].jX), sizeof(double));
            ofsCurrent.write(reinterpret_cast<const char*>(&host_current[index].jY), sizeof(double));
            ofsCurrent.write(reinterpret_cast<const char*>(&host_current[index].jZ), sizeof(double));
        }
    }
...
void PIC2D::calculateSecondMoments()
{
    momentCalculator.calculateSecondMomentOfOneSpecies(
        secondMomentIon, particlesIon, PIC2DConst::existNumIon
    );
    momentCalculator.calculateSecondMomentOfOneSpecies(
        secondMomentElectron, particlesElectron, PIC2DConst::existNumElectron
    );
}
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsZerothMomentIon.write(reinterpret_cast<const char*>(
                &host_zerothMomentIon[index].n), sizeof(double)
            );
        }
    }
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsZerothMomentElectron.write(reinterpret_cast<const char*>(
                &host_zerothMomentElectron[index].n), sizeof(double)
            );
        }
    }
}
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsFirstMomentIon.write(reinterpret_cast<const char*>(
                &host_firstMomentIon[index].x), sizeof(double)
            );
            ofsFirstMomentIon.write(reinterpret_cast<const char*>(
                &host_firstMomentIon[index].y), sizeof(double)
            );
            ofsFirstMomentIon.write(reinterpret_cast<const char*>(
                &host_firstMomentIon[index].z), sizeof(double)
            );
        }
    }
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsFirstMomentElectron.write(reinterpret_cast<const char*>(
                &host_firstMomentElectron[index].x), sizeof(double)
            );
            ofsFirstMomentElectron.write(reinterpret_cast<const char*>(
                &host_firstMomentElectron[index].y), sizeof(double)
            );
            ofsFirstMomentElectron.write(reinterpret_cast<const char*>(
                &host_firstMomentElectron[index].z), sizeof(double)
            );
        }
    }
}
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].xx), sizeof(double)
            );
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].yy), sizeof(double)
            );
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].zz), sizeof(double)
            );
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].xy), sizeof(double)
            );
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].xz), sizeof(double)
            );
            ofsSecondMomentIon.write(reinterpret_cast<const char*>(
                &host_secondMomentIon[index].yz), sizeof(double)
            );
        }
    }
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].xx), sizeof(double)
            );
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].yy), sizeof(double)
            );
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].zz), sizeof(double)
            );
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].xy), sizeof(double)
            );
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].xz), sizeof(double)
            );
            ofsSecondMomentElectron.write(reinterpret_cast<const char*>(
                &host_secondMomentElectron[index].yz), sizeof(double)
            );
        }
    }
}
...
)
{
    ...
    for (unsigned long long i = 0; i < PIC2DConst::existNumIon; i++) {
        ...
        ofsXIon.write(reinterpret_cast<const char*>(&x), sizeof(double));
        ofsXIon.write(reinterpret_cast<const char*>(&y), sizeof(double));
        ofsXIon.write(reinterpret_cast<const char*>(&z), sizeof(double));
    ...
    for (unsigned long long i = 0; i < PIC2DConst::existNumElectron; i++) {
        ...
        ofsXElectron.write(reinterpret_cast<const char*>(&x), sizeof(double));
        ofsXElectron.write(reinterpret_cast<const char*>(&y), sizeof(double));
        ofsXElectron.write(reinterpret_cast<const char*>(&z), sizeof(double));
    ...
    ofsNumIon.write(reinterpret_cast<const char*>(&PIC2DConst::existNumIon), sizeof(unsigned long long));
    ofsNumElectron.write(reinterpret_cast<const char*>(&PIC2DConst::existNumElectron), sizeof(unsigned long long));
}

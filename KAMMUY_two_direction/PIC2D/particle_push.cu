...
)
{
    pushPositionOfOneSpecies(
        particlesIon, PIC2DConst::existNumIon, dt
    );
    pushPositionOfOneSpecies(
        particlesElectron, PIC2DConst::existNumElectron, dt
    );
}
...
)
{
    ...
    xOverDx = (particle.x - PIC2DConst::device_xmin) / PIC2DConst::device_dx;
    yOverDy = (particle.y - PIC2DConst::device_ymin) / PIC2DConst::device_dy;

    xIndex1 = floor(xOverDx);
    xIndex2 = xIndex1 + 1;
    xIndex2 = (xIndex2 == PIC2DConst::device_nx) ? 0 : xIndex2;
    yIndex1 = floor(yOverDy);
    yIndex2 = yIndex1 + 1;
    yIndex2 = (yIndex2 == PIC2DConst::device_ny) ? 0 : yIndex2;

    if (xIndex1 < 0 || xIndex1 >= PIC2DConst::device_nx) printf("x = %f, index = %d, ERROR\n", particle.x, xIndex1); 

...
void PIC2D::initialize()
{
    ...
    for (int i = 0; i < PIC2DConst::nx; i++) {
        for (int j = 0; j < PIC2DConst::ny; j++) {
            ...
            int ni = PIC2DConst::numberDensityIon * (1.0 + pow(cosh((y - yCenter) / sheatThickness), -2));
            int ne = PIC2DConst::numberDensityElectron * (1.0 + pow(cosh((y - yCenter) / sheatThickness), -2));
            ...
            double p = PIC2DConst::B0 * PIC2DConst::B0 / 2.0 / PIC2DConst::mu0 * (beta + pow(cosh((y - yCenter) / sheatThickness), -2)); 
            double vThIon = sqrt(p / PIC2DConst::mIon / ni); 
            double vThElectron = sqrt(p / PIC2DConst::mElectron / ne); 

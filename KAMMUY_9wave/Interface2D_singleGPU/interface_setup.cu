...
)
{
    ...
    if (i < localNxMHD && j < PIC2DConst::device_ny / Interface2DConst::device_gridSizeRatio) {
        ...
        averagedB                    = getAveragedFieldForPICtoMHD(B_timeAve, i, j);
        averagedZerothMomentIon      = getAveragedFieldForPICtoMHD(zerothMomentIon_timeAve, i, j);
        averagedZerothMomentElectron = getAveragedFieldForPICtoMHD(zerothMomentElectron_timeAve, i, j);
        averagedFirstMomentIon       = getAveragedFieldForPICtoMHD(firstMomentIon_timeAve, i, j);
        averagedFirstMomentElectron  = getAveragedFieldForPICtoMHD(firstMomentElectron_timeAve, i, j);
        averagedSecondMomentIon      = getAveragedFieldForPICtoMHD(secondMomentIon_timeAve, i, j);
        averagedSecondMomentElectron = getAveragedFieldForPICtoMHD(secondMomentElectron_timeAve, i, j);

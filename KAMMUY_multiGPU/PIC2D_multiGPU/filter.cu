...
)
{
    ...
    if ((0 < i) && (i < localSizeX - 1) && (0 < j) && (j < localSizeY - 1)) {
        ...
        F[index].F = ((E[index].eX - E[index - localSizeY].eX) / PIC2DConst::device_dx 
...
)
{
    ...
    if (i < existNumSpecies) {
        ...
        yOverDy = (particlesSpecies[i].y - yminForProcs + buffer * PIC2DConst::device_dy) / PIC2DConst::device_dy;
